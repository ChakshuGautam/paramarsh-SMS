 ...
 */
export async function queryParamarshSMS(options) {
    const { query, module, branchId = DEFAULT_BRANCH, limit = 20 } = options;
    try {
        const apiCalls = mapQueryToAPICalls(query, module, branchId, limit);
        const results = await Promise.all(apiCalls.map(call => executeAPICall(call, branchId)));
 ...
 */
function mapQueryToAPICalls(query, module, branchId, limit) {
 ...
 */
function mapStudentQueries(query, limit) {
    const calls = [];
 ...
 */
function mapTeacherQueries(query, limit) {
    const calls = [];
 ...
 */
function mapAttendanceQueries(query, limit) {
    const calls = [];
 ...
 */
function mapInvoiceQueries(query, limit) {
    const calls = [];
    // Status-based queries
 ...
 */
function mapCommunicationQueries(query, limit) {
    const calls = [];
    // Message queries
 ...
 */
function mapGeneralQueries(query, limit) {
    const calls = [];
 ...
 */
async function executeAPICall(apiCall, branchId) {
 ...
 */
function formatResults(query, apiCalls, results) {
    ...
    results.forEach((result, index) => {
        ...
        if (result.data && Array.isArray(result.data)) {
            if (result.data.length === 0) {
                ...
                result.data.slice(0, displayCount).forEach((record, i) => {
