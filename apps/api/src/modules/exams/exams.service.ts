...
export type Exam = { name: string; startDate?: string; endDate?: string };
...
export class ExamsService {
  ...
  constructor() {
    ...
    const csvPath = path.join(root, 'docs/Seeds/exams.csv');
    if (fs.existsSync(csvPath)) {
      const lines = fs
        ...
        .filter(Boolean)
        .slice(1);
      this.exams = lines.map((l) => {
        const [name, startDate, endDate] = l.split(',');
        return { name, startDate, endDate } as Exam;
      });
    }
  }
