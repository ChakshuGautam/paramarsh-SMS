  ...
  useEffect(() => {
    if (record && sections.length > 0) {
      ...
      if (section) {
        ...
        fetchGradeAppropriateSubjects(section.class.id);
        fetchSubjectTeacherMappings(section.class.id);
      }
    }
  }, [record, sections]);
    ...
    <Edit>
      <SimpleForm 
        onSubmit={async (data) => {
          ...
          try {
            await dataProvider.update('timetables', {
              id: record.id,
              data: {
                ...data,
            ...
            notify('Timetable entry updated successfully', { type: 'success' });
            redirect('/admin/timetables');
          } catch (error) {
            notify('Error updating timetable entry', { type: 'error' });
          }
        }}
      >
        <div className="space-y-6">
          ...
          {record && (
            <Alert>
              ...
              <AlertDescription>
                <div className="font-medium mb-1">Editing Timetable Entry</div>
                <div className="text-sm">ID: {record.id} | Created: {new Date(record.createdAt).toLocaleDateString()}</div>
              </AlertDescription>
            </Alert>
          )}
